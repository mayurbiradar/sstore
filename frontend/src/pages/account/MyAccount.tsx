...
import { getMyOrders, getMyAddresses, type Order, type Address } from '../../api/orderApi'
...
export default function MyAccount() {
  ...
  const [editData, setEditData] = useState<EditProfileForm>({ firstName: '', lastName: '', email: '', phone: '' })
  ...
  const loadOrders = async () => {
    setBusy(true);
    try {
      const loaded = await getMyOrders(getStoredAccessToken() || undefined);
      setOrders(loaded || []);
    } finally {
      setBusy(false);
    }
  ...
  const loadAddresses = async () => {
    ...
    setBusy(true);
    try {
      const loaded = await getMyAddresses(getStoredAccessToken() || undefined);
      setAddresses(loaded);
    } catch {
      setAddresses([]);
    } finally {
  ...
  const saveProfile = async () => {
    setBusy(true)
    try {
      const updated = await updateMyProfile(editData, getStoredAccessToken() || undefined)
...
function Overview({ user, onEdit }: { user: User; onEdit: () => void }) {
  return <div>
    <SectionHeading title={`Hello, ${user.firstName || 'there'}`} description={`Manage your account and keep track of your ${STORE.name} activity.`} />
