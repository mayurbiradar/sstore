...
export default function MyAccount() {
  ...
  const loadOrders = async () => {
    setBusy(true);
    try {
      const loaded = await getMyOrders(localStorage.getItem('accessToken') || undefined);
      setOrders(loaded || []);
    } finally {
      setBusy(false);
    }
  ...
  const loadAddresses = async () => {
    ...
    setBusy(true);
    try {
      const loaded = await getMyAddresses(localStorage.getItem('accessToken') || undefined);
      setAddresses(loaded);
    } catch {
      setAddresses([]);
    } finally {
  ...
  const saveProfile = async () => {
    setBusy(true)
    try {
      const updated = await updateMyProfile(editData, localStorage.getItem('accessToken') || undefined)
