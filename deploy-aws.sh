...
command -v doppler >/dev/null 2>&1 || {
    echo "Doppler CLI is required. Install it and run 'doppler setup' first." >&2
    exit 1
}

[[ -n "$EC2_HOST" ]] || {
    echo "EC2_HOST is required, for example: EC2_HOST=203.0.113.10 ./deploy-aws.sh" >&2
    exit 1
}
...
scp -i "$KEY_FILE" docker-compose.aws.yml \
...
# Learning deployment: download the configured Doppler environment locally and
# copy it to the temporary EC2 instance. No .env file is written locally.
...
. ./.env
set +a
[[ -n "\${AWS_HOSTNAME:-}" ]] || { echo "AWS_HOSTNAME is missing from the Doppler environment" >&2; exit 1; }
