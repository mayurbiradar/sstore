...
export default function OrderSummary({
  ...
  return (
    <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-24">
      ...
      <FreeShippingProgress subtotal={subtotal} formatPrice={formatPrice} freeShippingPaise={freeShippingPaise} />
      ...
      <div className="mt-6 space-y-3 border-t border-slate-200 pt-5 text-sm">
        ...
        <SummaryRow label="GST (3%)" value={formatPrice(tax)} />
        {discount > 0 && (
          <SummaryRow
            label="Discount"
...
function FreeShippingProgress({
  ...
  const remaining = freeShippingPaise - subtotal
  const progress = Math.min(100, (subtotal / freeShippingPaise) * 100)
  return (
    <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm">
      <div className="flex items-center justify-between text-slate-600">
        <span className="font-semibold">Add {formatPrice(remaining)} more</span>
