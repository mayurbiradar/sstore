...
export default function OrderSuccess() {
  ...
  useEffect(() => {
    ...
    if (sessionId && orderId && !initialOrder) {
      setPaymentLoading(true)
      confirmStripePayment(orderId, sessionId, localStorage.getItem('accessToken') || undefined)
        .then(response => { setOrder(response.data); clearCart() })
        .catch(() => setOrder(null))
  ...
  const stripePayment = new URLSearchParams(location.search).has('session_id')
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
          ...
          <div className="grid gap-4 border-b border-slate-200 py-5 sm:grid-cols-3">
            ...
            <Detail label="Payment" value={stripePayment ? 'Paid securely with Stripe' : 'Cash on delivery'} />
            <Detail label="Delivery" value="3-5 business days" />
          </div>

          {order?.items?.length > 0 && <div className="border-b border-slate-200 py-5">
            <h2 className="text-lg font-black text-slate-950">Items in your order</h2>
            <div className="mt-4 space-y-4">
              {order.items.map((item: any) => <div key={item.id || item.productId} className="flex items-center gap-3"><div className="h-12 w-12 flex-shrink-0 overflow-hidden rounded-lg bg-slate-100">{item.image && <img src={item.image.startsWith('/images/') ? `${API_BASE_URL}${item.image}` : item.image} alt={item.productName} className="h-full w-full object-cover" />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-slate-900">{item.productName}</p><p className="mt-1 text-sm text-slate-500">Qty {item.quantity}</p></div><p className="text-sm font-bold text-slate-900">₹{item.subtotal?.toLocaleString('en-IN')}</p></div>)}
            </div>
          </div>}
          <div className="flex items-center justify-between pt-5"><span className="text-lg font-black text-slate-950">{stripePayment ? 'Total paid' : 'Total paid on delivery'}</span><span className="text-2xl font-black text-rose-600">₹{order?.totalAmount?.toLocaleString('en-IN') || '0'}</span></div>
        </section>
        ...
        <p className="mt-6 text-center text-xs text-slate-400">Need help? support@akshatasjewelbox.com · +91-1234-567-890</p>
      </div>
    </main>
  )
}
